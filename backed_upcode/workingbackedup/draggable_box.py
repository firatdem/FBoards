...
class DraggableBox:
    def __init__(self, app, canvas, text, role, x, y, phone=None, job_site=None, box=None, skills=None, sst_card="No",
                 nj_ny_certified="No", electrician_rank="1", certifications=None, worker_status=None):
        self.app = app
        self.canvas = canvas
        self.text = text
        self.role = role
    ...
    def get_snap_box_left_top(self, hub, box):
        ...
        elif box in ["Electrician", "Fire Alarm", "Fire Alarm Electrician", "Roughing Electrician"]:
            coords = self.canvas.coords(hub.electrician_box)  # Assuming they all snap to the same box for simplicity
        else:
            coords = [0, 0, 0, 0]
        left_x = coords[0]
        top_y = coords[1]
